...
class PxeLegacyBoot(BaseTest):
  """CdromLegacyBoot class"""
  NAME = "pxe_legacy_boot"
  TAGS = ["boot", "legacy", "pxe"]
  ...
  POST_OPERATIONS = ["setup_rpc", "verify_os_boot", "set_boot_disk",
                     ...
                     "check_device_status", "prepare_for_vnic_hot_add"]
  #, "pxe_teardown"]
  DEFAULT_PARAMS = {
    "memory": 2048,
    "vcpus": 2,
  ...
  def teardown(self, **params):
    ...
    cache = modules.get("cache")
    vm_name = extra_params.get("vm_name")
    pxe_vm = cache.get_entity(entity_type="pxe_vm", name=vm_name)
    pxe_vm.remove()
