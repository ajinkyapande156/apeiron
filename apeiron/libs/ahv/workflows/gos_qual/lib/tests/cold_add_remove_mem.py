...
class ColdAddRemoveMemory(BaseTest):
  ...
  def run(self, **params):
    ...
    INFO("Powering off VM: %s for memory cold add" % vm.name)
    vm.power_off()
    INFO("Adding %s memory" % params.get("memory"))
    vm.edit(**params)
    res = vm.get()
    assert params.get("memory") == res.get("memory_mb"), \
      "Failed to cold add memory via RESTv2"
    ...
    assert mem == math.ceil(float(params.get("memory")) / float(1024)), \
      ...
      "Found: [%s]" % (math.ceil(float(params.get("memory")) / float(1024)),
                       mem)
    INFO("Cold add memory completed successfully for VM: %s"
         % vm.name)

    INFO("Powering off VM: %s for memory cold remove" % vm.name)
    vm.power_off()
